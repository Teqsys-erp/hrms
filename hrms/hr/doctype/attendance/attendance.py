...
class Attendance(Document):
     ...
     def validate(self):
          ...
          validate_status(self.status, ["Present", "Absent", "On Leave", "Half Day", "Work From Home", "Outdoor"])
...
def add_attendance(filters):
    ...
    for record in attendance:
        ...
        if status == "On Leave":
            # Show only leave type for "On Leave"
            leave_type = record.get("leave_type") or "-"
            record["title"] = (
                f"{record.employee_name} : {status}\n"
                f"Leave: {leave_type}"
            )

        elif status == "Half Day":
            # Show leave type along with in_time, out_time, and working_hours
            leave_type = record.get("leave_type") or "-"
            in_time = frappe.utils.format_time(record.get("in_time")) if record.get("in_time") else "-"
            out_time = frappe.utils.format_time(record.get("out_time")) if record.get("out_time") else "-"
            ...
            # Show attendance details for other statuses
            in_time = frappe.utils.format_time(record.get("in_time")) if record.get("in_time") else "-"
            out_time = frappe.utils.format_time(record.get("out_time")) if record.get("out_time") else "-"
            working_hours = record.get("working_hours") or "-"
    ...
    return attendance  # ✅ This line must be at base indentation level
